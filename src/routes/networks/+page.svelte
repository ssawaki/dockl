<script lang="ts">
  import { formatError } from "$lib/errors";
  import { listNetworks, removeNetwork, pruneNetworks } from "$lib/ipc/networks";
  import { connection } from "$lib/stores/connection";
  import { refreshOnDockerEvents } from "$lib/dockerEvents.svelte";
  import { refreshOnF5 } from "$lib/shortcuts.svelte";
  import ConfirmDialog from "$lib/components/ui/ConfirmDialog.svelte";
  import Icon from "$lib/components/ui/Icon.svelte";
  import PageHeader from "$lib/components/layout/PageHeader.svelte";
  import MasterDetail from "$lib/components/layout/MasterDetail.svelte";
  import NetworkMasterList from "$lib/components/networks/NetworkMasterList.svelte";
  import NetworkDetailPanel from "$lib/components/networks/NetworkDetailPanel.svelte";
  import { pushToast, resolveToast } from "$lib/stores/toasts";
  import { get } from "svelte/store";
  import { t } from "$lib/stores/i18n";
  import broomIcon from "@fluentui/svg-icons/icons/broom_16_regular.svg?raw";
  import arrowClockwiseIcon from "@fluentui/svg-icons/icons/arrow_clockwise_16_regular.svg?raw";
  import type { NetworkSummary } from "$lib/types";

  let networks = $state<NetworkSummary[]>([]);
  let loading = $state(true);
  let errorMessage = $state<string | null>(null);
  let pruning = $state(false);

  let selectedId = $state<string | null>(null);
  // Resolved from the live list rather than stored, so a refresh (or a removal) can't
  // leave the panel showing something that has since changed or gone.
  let selectedNetwork = $derived(networks.find((n) => n.id === selectedId) ?? null);

  async function refresh() {
    try {
      networks = await listNetworks();
      errorMessage = null;
    } catch (e) {
      errorMessage = formatError(e);
    } finally {
      loading = false;
    }
  }

  // The root layout runs the actual WSL2/Docker connection check once at app startup;
  // this just loads this page's own data once that's confirmed, then reacts to
  // `network` events (create/destroy) instead of polling.
  refreshOnDockerEvents(() => $connection.status === "connected", ["network"], refresh);
  refreshOnF5(refresh);

  let confirmTarget = $state<NetworkSummary | null>(null);

  function requestRemove(network: NetworkSummary) {
    confirmTarget = network;
  }

  function closeConfirm() {
    confirmTarget = null;
  }

  async function confirmRemove() {
    const network = confirmTarget;
    if (!network) return;
    closeConfirm();

    const name = network.name;
    const toastId = pushToast(get(t)("toast.remove.pending", { name }));
    try {
      await removeNetwork(network.id);
      await refresh();
      resolveToast(toastId, "success", get(t)("toast.remove.success", { name }));
    } catch (e) {
      resolveToast(toastId, "error", get(t)("toast.remove.error", { name, error: formatError(e) }));
    }
  }

  let pruneConfirmOpen = $state(false);

  function requestPrune() {
    pruneConfirmOpen = true;
  }

  async function confirmPrune() {
    pruneConfirmOpen = false;
    pruning = true;
    const resource = get(t)("resource.networks");
    const toastId = pushToast(get(t)("prune.pending", { resource }));
    try {
      const summary = await pruneNetworks();
      await refresh();
      resolveToast(toastId, "success", summary.trim() || get(t)("prune.success", { resource }));
    } catch (e) {
      resolveToast(toastId, "error", get(t)("prune.error", { error: formatError(e) }));
    } finally {
      pruning = false;
    }
  }
</script>

<div class="page-view">
  <PageHeader title={$t("nav.networks")}>
    <!-- svelte-ignore a11y_click_events_have_key_events -->
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <fluent-button appearance="outline" tabindex="-1" disabled={pruning} onclick={requestPrune}>
      <span class="btn-content">
        {#if pruning}
          <fluent-spinner size="tiny"></fluent-spinner>
        {:else}
          <Icon svg={broomIcon} size={14} />
        {/if}
        {$t("prune.button", { resource: $t("resource.networks") })}
      </span>
    </fluent-button>
    <!-- svelte-ignore a11y_click_events_have_key_events -->
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <fluent-button
      appearance="outline"
      icon-only
      tabindex="-1"
      title={$t("common.refresh")}
      aria-label={$t("common.refresh")}
      onclick={refresh}
    >
      <Icon svg={arrowClockwiseIcon} size={14} />
    </fluent-button>
  </PageHeader>

  {#if errorMessage}
    <div class="error-banner dockl-surface">{errorMessage}</div>
  {/if}

  <!-- The spinner lives inside the list rather than replacing this whole area: only
       the list is waiting on anything, and blanking the detail panel too would throw
       away what the user was reading. -->
  <MasterDetail>
    {#snippet list()}
      <NetworkMasterList {networks} {loading} bind:selectedId onRemove={requestRemove} />
    {/snippet}
    {#snippet detail()}
      <NetworkDetailPanel network={selectedNetwork} />
    {/snippet}
  </MasterDetail>
</div>

{#if confirmTarget}
  <ConfirmDialog
    title={$t("networks.confirmRemove.title")}
    message={$t("confirmRemove.messageInUse", { name: confirmTarget.name })}
    onConfirm={confirmRemove}
    onCancel={closeConfirm}
  />
{/if}

{#if pruneConfirmOpen}
  <ConfirmDialog
    title={$t("prune.button", { resource: $t("resource.networks") })}
    confirmLabel={$t("action.prune")}
    message={$t("networks.prune.message")}
    onConfirm={confirmPrune}
    onCancel={() => (pruneConfirmOpen = false)}
  >
    {#snippet extra()}
      <code class="command-preview">docker network prune -f</code>
    {/snippet}
  </ConfirmDialog>
{/if}

<style>
  .command-preview {
    display: block;
    margin-top: 10px;
    padding: 6px 8px;
    font-size: 12px;
    font-family: Consolas, "Cascadia Code", monospace;
    color: var(--dockl-text-secondary);
    background: var(--dockl-surface-hover);
    border-radius: 4px;
    white-space: nowrap;
    overflow-x: auto;
  }

  .btn-content {
    display: inline-flex;
    align-items: center;
    gap: 6px;
  }

  .error-banner {
    padding: 8px 12px;
    color: var(--dockl-danger);
    border-color: var(--dockl-danger);
  }
</style>
